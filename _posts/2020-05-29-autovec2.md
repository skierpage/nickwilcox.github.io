...
In the [previous article]({% post_url 2020-05-16-autovec %}) on auto-vectorization we treated instructions as either SIMD (**S**ingle **I**instruction **M**ultiple **D**ata) or non-SIMD. We also assumed that SIMD meant four values at a time. 
...
To break what we've done to our code:
...
Unfortunately the answer seems to be no. With the current version of Rust (1.43.0) it seems that the AVX performance is worse than SSE2. AVX2 is a slight win, but in testing with machines other than my main laptop it was also slower than the SSE laptop.
...
The thing to note about this code are that all the intrinsics we use start with `_mm256` to indicate they are from the AVX or AVX2 instruction family. We've also had to keep the `#[target_feature(enable = "avx2")]` attribute on our function to ensure the compiler generates the correct code for our intrinsics.
